...
Section code.
  ...
  Definition grow : val :=
	λ: "deque",
	  ...
	  let: "b" := !(bot "deque") in
	  let: "nsz" := #2 * "sz" in
	  let: "narray" := AllocN "nsz" #0 in
	  let: "loop" := (rec: "loop" "i" :=
		if: "i" ≤ "b" then (
		  let: "v" := ("array" +ₗ ("i" `rem` "sz")) in
		  "narray" +ₗ ("i" `rem` "nsz") <- "v" ;;
		  "loop" ("i" + #1)
		) else #()
	  ) in
	  "loop" "t" ;;
  ...
  Proof with extended_auto.
    iIntros "#Inv Own" (Φ) "AU".
      iDestruct "Own" as (γq γpop γm arr top bot b l)
        "(%Enc & -> & γOwn & bOwn & arrOwn)".
      iDestruct "Inv" as (sz arr' top' bot') "[%Q Inv]".
      injection Q as [= Hsz <- <- <-].
    ...
    { wp_pures. wp_bind (grow _)%E.
	  awp_apply (grow_spec with "[] [γOwn bOwn arrOwn]").
	    { rewrite Hsz. fr. } { fr. }
